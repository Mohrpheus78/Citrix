...
Do{
   #Reset Timeouts
	...
	sleep -seconds 10
	Write-Host "Checking for new updates available on '$MaintDeviceName'"`n
...
IF ($BISF -eq "YES") {
	...
	$CacheDrive = Invoke-Command -ComputerName $MaintDeviceName -ScriptBlock {(Get-WmiObject -Class Win32_logicaldisk -Filter "DriveType = '3'" | Where-Object {$_.DeviceID -ne "C:"}).DeviceID}
	$CacheDrive = $CacheDrive -replace (":","$")
	$BISFLogLocation = Invoke-Command -ComputerName $MaintDeviceName -ScriptBlock {
		IF (Get-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Login Consultants\BISF" -Name LIC_BISF_CLI_LS -ErrorAction SilentlyContinue) {
		Get-ItemPropertyValue -Path "HKLM:\SOFTWARE\Policies\Login Consultants\BISF" -Name LIC_BISF_CLI_LS
		}
	}
	...
	IF ($BISFLogLocation) {
		Do {
			$BISFLog = (Get-ChildItem -Path "$BISFLogLocation\$MaintDeviceName" | Select-Object -Last 1).Name
	...
	ELSE {
		Do {
			$BISFLog = (Get-ChildItem -Path "\\$MaintDeviceName\$CacheDrive\BISFLogs" | Select-Object -Last 1).Name
...
IF ($Hypervisor -eq "Xen") {
	Do {
		Write-Host "VM '$MaintDeviceName' is shutting down..."`n
